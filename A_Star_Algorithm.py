...
# 封闭列表close_list_C，用于存放已被操作过的点，不能再被操作，该点的值应该用2赋值，表示障碍
...
def init(start_position, end_position):
    ...
    # 若open_list_S[i][j] = 0，表示没有保存数据，不能被操作
    # 若open_list_S[1][1] = 1，表示坐标为（1,1）的点在open_list_S中可以被操作
    for i in range(1, n + 1):
        for j in range(1, n + 1):
            open_list_S[i][j] = 0

    # 若close_list_C[i][j] = 0，表示没有保存数据，可以被考察
    # 若close_list_C[1][1] = 1，表示坐标为（1,1）的点在close_list_C中，不再被考察
    for i in range(1, n + 1):
        for j in range(1, n + 1):
            close_list_C[i][j] = 1
...
if __name__ == "__main__":
    print "program begin……"
    # 坐标自己定
    start_position = [7, 7]  # 起始坐标
    end_position = [9, 7]  # 目标坐标
    # 程序初始化
    ...
    while end_position[0] != parent[0] or end_position[1] != parent[1]:
        ...
        create_2D_space[parent[0]][parent[1]] = 2
        # 从当前找出的最短距离的点进行操作
        parent[0] = result[1]
        parent[1] = result[2]
